...
abstract class sfFormObject extends BaseForm
{
  ...
  {
    $text = preg_replace_callback('#/(.?)#',       function ($matches) { return '::'.strtoupper($matches[1]);}, $text);
    $text = preg_replace_callback('/(^|_|-)+(.)/', function ($matches) { return strtoupper($matches[2]);}, $text);
